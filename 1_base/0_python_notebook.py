...
3.game			#compile python files on linux : ->touch new.py -> python3 new.py
...
5.BIF = Bulit-in-Function	##nei zhi han shu
...
13. type()	#This function can get the type of value
		#type()函數
eg:
a = '5'
type(a)		##class 'str', string varible

14. isinstance()	##Judge whether two type of value is same. @return:ture false    ,bool
a = 5
isinstance(a, int)	#return true
			#int is a class, a shi lei de shi li

15. operator: +  -  *  /  %  **(mi yun suan,ci fang, ping fang)  //(float division, )
a += 3    # +=   -=   *=  /=
eg:
3/2 	#result = 1.5
3//2	#result = 1
3**2	#result = 3 * 3 = 9
(-3)*(-3)  	#if exist negative number in your program.You'd better add (bracket)
		#如果乘法中存在負數，最好加上括號(bracked)

16. Logic operator  and  or  not

17. elif time > 5:	#elif = else if
...
mix[1:3]	#The Third element is not included
		#下標: 1 <= i < 3 (包前不包後)
...
'abc' not in list2	# return false

24. list[1][1]		#列表中間嵌套列表，獲取第二個列表中的值
mix = [1, 2.34, 'list', [1,2,3]]
mix[3][1]	#mix[3][1] = 2	Attention!!!

25. dir(list)		#look for the function inside list, show all the list's BIF 
			#顯示list的所有內置方法（BIF）
...
26. The difference between list and tuple(列表和元組的區別)
...
eg:str1.center(40)	#center of 40 
...
eg:str1.index('sub')	#return :ValueError: substring not found
...
enumerate()	#return a literator too
eg:
